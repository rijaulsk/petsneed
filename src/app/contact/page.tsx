...
const faqs = [
    { question: "What's the best way to reach you?", answer: "For immediate assistance, call us at 7411576879. For non-urgent questions, you can also WhatsApp us or email contact@petsneed.in." },
    { question: "Do I need an appointment?", answer: "For grooming and shopping, walk-ins are welcome daily from 10 AM to 10 PM. For veterinary services, please call ahead to confirm doctor availability." },
    { question: "Is parking available?", answer: "Yes, parking is available near our location on Diamond Harbour Road, Alipore, Kolkata." },
    { question: "How do I get directions?", answer: "Click 'Get Directions' on our contact cards or map, or search \"Pet's Need Alipore\" on Google Maps. We're easily accessible from all parts of South Kolkata." },
];
  ...
  return (
    <>
      <section className="relative bg-accent/20 pt-16 md:pt-24 pb-12 md:pb-20 overflow-hidden">
        <PetIcon as={Mail} className="top-24 left-10 h-24 w-24 -rotate-12 opacity-50" />
        <PetIcon as={MessageCircle} className="bottom-24 right-10 h-20 w-20 rotate-12 opacity-50" />
        <div className="container mx-auto px-4 md:px-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <AnimatedSection className="space-y-6 text-center lg:text-left">
                    <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tighter">Come Say Hi!</h1>
                    <p className="text-lg md:text-xl text-foreground/70 max-w-xl mx-auto lg:mx-0">
                        Visit us in Alipore, Kolkata, give us a call, or send a message. We're here to help with all your pet care needs, from expert advice to scheduling an appointment. We look forward to welcoming you and your furry friend!
                    </p>
                    <Button asChild size="lg" className="font-button"><a href={siteConfig.mapLink} target="_blank" rel="noopener noreferrer">Get Directions</a></Button>
                </AnimatedSection>
      ...
      <PageShell>
                ...
                {quickContact.map((contact, index) => (
                    <AnimatedSection as="div" key={contact.title} delay={index * 100}>
                        <Card className="text-center group hover:bg-primary/10 hover:border-primary transition-all duration-300 hover:-translate-y-1 h-full">
                            <a href={contact.href} target={contact.href.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="block p-6">
                                ...
                                <p className="text-xs text-foreground/60">{contact.label}</p>
                            </a>
                        </Card>
                    </AnimatedSection>
                ))}
        ...
        {/* Store Hours & Map */}
        <AnimatedSection as="section" className="mt-16 md:mt-24 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
                <h2 className="text-3xl font-bold mb-6">Store Hours & Location</h2>
                <div className="space-y-6">
                    <Card>
                        <CardContent className="p-6">
                            <h3 className="font-bold text-lg">Store, Grooming & Pharmacy</h3>
                            <p className="text-2xl font-semibold text-primary">Open Daily: 10:00 AM - 10:00 PM</p>
                        </CardContent>
                    </Card>
                    <Card className="bg-amber-50 border-amber-200">
                        <CardContent className="p-6">
                            <h3 className="font-bold text-lg text-amber-900">Veterinary Clinic</h3>
                            <p className="text-lg font-semibold text-amber-800">Please call before visiting</p>
                             <p className="text-sm text-amber-700">Doctor availability varies. Call <a href={`tel:${siteConfig.phone}`} className="font-bold underline">{siteConfig.phone}</a> to confirm.</p>
                        </CardContent>
                    </Card>
                </div>
            </div>
